...
tl::expected<std::vector<Triangle>, std::string_view> LoadGeometry(
  ...
  std::vector<std::vector<glm::vec3>> normalVertices(
      u_coords, std::vector<glm::vec3>(v_coords));
...
tl::expected<std::vector<std::vector<std::vector<int>>>, std::string_view> LoadUv(
    const std::string& path_file) {
  std::fstream file(path_file, std::ios_base::in);
  if (!file.is_open()) {
    return tl::unexpected(std::format("Couldn't open file {}", path_file));
  ...
  std::vector<Triangle> triangles;
  std::vector<std::vector<std::vector<int>>> triangles_uv;

  for (int i = 0; i <= vertices.size() - 2; ++i) {
    for (int j = 0; j <= vertices[i].size() - 2; ++j) {
      triangles_uv.push_back({{i,j}, {i, j+1}, {i+1, j}});
      triangles_uv.push_back({{i,j+1}, {i+1, j+1}, {i+1, j}});
    }
  }
